...
class find(main_intent):
    ...
    def generate_response(self, new_ent=None, new_trait=None):
        ...
        if 'topic' in self.entities:
            ...
            r = erequests.async.get(url + 'query=' + self.end_location + '&open' + '&key=' + self.g_api_key)
            r = list(erequests.map([r]))
...
class information(main_intent):
    ...
    def webcrawl(self, u):
        if u == 'rona':
            ...
            response = erequests.async.get(url)
            x = list(erequests.map([response]))
